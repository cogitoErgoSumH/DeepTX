...
hist_yobs_list = sim_results_pro_test[1:20]
hist_yobs_list = []
for k in 1: size(gene_exp_data)[2]
    push!(hist_yobs_list,convertCountsToProb(gene_exp_data[!,k]))
end

params_list, temp_loss_list = TX_inferrers(hist_yobs_list)

params_num = 20
stats_es_true = calculate_stats(params_list,params_arr_test[1:20])
cor(stats_es_true[!,"mean_es"],stats_es_true[!,"mean_true"])
cor(stats_es_true[!,"bf_es"],stats_es_true[!,"bf_true"])
...
estimated_params = inference_parameters(gene_exp_data,model)
estimated_params = matrix_to_dataframe(estimated_params,names(gene_exp_data))
estimated_params = calculate_stats(gene_exp_data,estimated_params)
